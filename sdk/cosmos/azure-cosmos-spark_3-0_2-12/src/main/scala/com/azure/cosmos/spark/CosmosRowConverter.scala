...
private object CosmosRowConverter
  ...
  private[spark] def fromObjectNodeToInternalRow(schema: StructType, objectNode: ObjectNode): InternalRow = {
    val row = fromObjectNodeToRow(schema, objectNode)
    RowEncoder(schema).createSerializer().apply(row)
  }

  private[spark] def fromObjectNodeToRow(schema: StructType, objectNode: ObjectNode): Row = {
    val values: Seq[Any] = convertStructToSparkDataType(schema, objectNode)
    new GenericRowWithSchema(values.toArray, schema)
  }

  private[spark] def fromRowToObjectNode(row: Row): ObjectNode = {

    if (row.schema.contains(StructField(CosmosTable.RAW_JSON_BODY_ATTRIBUTE_NAME, StringType))) {
      // Special case when the reader read the rawJson
      val rawJson = row.getAs[String](CosmosTable.RAW_JSON_BODY_ATTRIBUTE_NAME)
  ...
  private[spark] def fromInternalRowToObjectNode(row: InternalRow, schema: StructType): ObjectNode = {
    val objectNode: ObjectNode = objectMapper.createObjectNode()
  ...
  private def convertStructToSparkDataType(schema: StructType, objectNode: ObjectNode): Seq[Any] =
    schema.fields.map {
      case StructField(CosmosTable.RAW_JSON_BODY_ATTRIBUTE_NAME, StringType, _, _) =>
        objectNode.toString
